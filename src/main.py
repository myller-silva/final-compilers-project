...
grammar = [
    ...
    ("Condicional", [KW_SE, "ExpressaoLogica", KW_ENTAO, "Comandos", KW_SENAO, "Comandos", KW_FIM_SE]),
...
for input_file in inputs:
    ...
    print("\n-- Tokens --"," ".join([f"{token[1]}" for token in tokens]))
    parser = RecursiveDescentParser(grammar, "Programa", debug=False)
    abstract_syntax_tree = parser.parse(tokens)
    print_tree_nice(abstract_syntax_tree)
    
    #salvar árvore sintática abstrata em um arquivo
    output_file = input_file.replace("inputs", "outputs").replace(".txt", "_ast.txt")
    with open(output_file, "w") as file:
        file.write(str(abstract_syntax_tree))
